...
namespace WinShareFixer
{
    ...
    public class MainForm : Form
    {
        ...
        private string FormatTimeoutText(int timeoutMilliseconds)
        {
            ...
            {
                return string.Format("（超过 {0} 分钟）", timeoutMilliseconds / 60000);
            }
            else
            {
                return string.Format("（超过 {0} 秒）", timeoutMilliseconds / 1000);
            }
        }
        #endregion
        #region 事件处理
        private void BtnGuestFix_Click(object sender, EventArgs e)
        {
            ...
            ResetLogView();
            LogSystemInformation();
            var timeoutTimer = new System.Threading.Timer(delegate(object state)
            {
                CancelCurrentOperation();
                Log("\n        [警告] 操作超时" + FormatTimeoutText(600000) + "，已强制停止。");
                SetStatusText("状态：操作超时，请重试。");
                FinishOperation(cts);
            }, null, 600000, System.Threading.Timeout.Infinite);
            lock (_operationLock)
            {
                _currentTimeoutTimer = timeoutTimer;
            }
        ...
        private void BtnPauseUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                CancelCurrentOperation();
                Log("\n        [警告] 操作超时" + FormatTimeoutText(300000) + "，已强制停止。");
                SetStatusText("状态：操作超时，请重试。");
                FinishOperation(cts);
        ...
        #endregion
        #region 1. Guest 修复逻辑
        private void RunGuestFixLogic(CancellationToken cancellationToken)
        {
            Log("==================================================");
            ...
            RemoveGuestFromDenyLogon(cancellationToken);
            int build = GetWindowsBuildNumber();
            if (build < 9200)
            {
        ...
        #endregion
        #region 2. SMB 修复逻辑
        private void RunSmbFixLogic(CancellationToken cancellationToken)
        {
            Log("==================================================");
        ...
        #endregion
        #region 3. 打印机修复逻辑
        private void RunPrinterFixLogic(CancellationToken cancellationToken)
        {
            Log("==================================================");
        ...
        #endregion
        #region 4. 暂停更新逻辑
        private void RunPauseUpdateLogic(CancellationToken cancellationToken)
        {
            Log("==================================================");
        ...
        private string RunPowerShellCommand(string cmd)
        {
            try
            {
                ...
                {
                    FileName = "powershell",
                    Arguments = "-NoProfile -OutputFormat Text -ExecutionPolicy Bypass -EncodedCommand " + encodedCommand,
                    UseShellExecute = false,
                ...
                using (Process p = Process.Start(psi))
                {
                    if (p != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(error) &&
                            ...
                            !error.Contains("<Objs") &&
                            !error.Contains("Microsoft.PowerShell") &&
        ...
        private void RemoveGuestFromDenyLogon(CancellationToken cancellationToken)
        {
            int build = GetWindowsBuildNumber();
            if (build < 9200)
            {
